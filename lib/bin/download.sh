...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    ...
    url="https://github.com/kamontat/tpr/releases/download/v{version}/kc-tpr_{version}_{os}_{arch}"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/kamontat/tpr/releases/download/v{version}/checksums_{version}.txt"
